...
\begin{figure}
	...
	%TODO INSERIR ENGINE BLOCK COM VARIAÇÃO DO SIGMA
	\caption{lalalalla}
	\label{}
\end{figure}
...
%	É preciso lembrar que a função $ t(v) $, assim como as funções $ g(v) $ e $ h(v) $ de \textit{Kindlmann e Durkin}~\cite{gordon}, raramente apresentarão exatamente o comportamento exibido nas Figuras \ref{fig:m_inflection} e \ref{fig:g_functions}. Nos volumes em que o intervalo de valores das fronteiras se sobrepõem, a média afeta o comportamento dos arcos esperados. Para demonstrar esse fenômeno, dois volumes sintéticos foram criados, cada um contendo duas esferas concêntricas, onde as fronteiras se comportam de forma ideal. A diferença entre os volumes está nos valores escolhidos para cada esfera, fazendo com que haja sobreposição no intervalo das fronteiras apenas em um deles. As Figuras \ref{fig:m_double_sphere} e \ref{fig:m_double_sphere_disc} exibem as fatias e as derivadas médias para os dois volumes.
%	
%	As esferas da Figura~\ref{fig:m_double_sphere} possuem fronteiras nos intervalos $ [0,127] $ e $ [127,255] $, como mostra a imagem~\ref{fig:m_double_sphere_slice}. Portanto, espera-se que o centro das fronteiras estejam próximos a $ 64 $ e $ 190 $. Em~\ref{fig:m_double_sphere_deriv} percebe-se que as derivadas médias se comportam exatamente como o esperado, pois $ 64 $ e $ 190 $ são justamente os pontos de primeira derivada máxima, segunda derivada zero e terceira derivada mínima.
%	
%	Já na Figura~\ref{fig:m_double_sphere_disc} os intervalos das fronteiras são $ [0,255] $ e $ [170,255] $. Percebe-se que devido à sobreposição de valores, as funções não se comportam como o esperado. Apesar de existir uma fronteira entre $ 0 $ e $ 255 $, as derivadas não exibem um arco único entre esses valores. A imagem~\ref{fig:m_double_sphere_disc_deriv} mostra que o arco é interrompido justamente no valor $ 170 $, que é o início da segunda fronteira e por isso, um arco destorcido se estende até $ 255 $.	Porém, como a Figura~\ref{fig:m_double_sphere_disc}~\ref{fig:m_double_sphere_disc_deriv} ilustra, os mínimos locais em $ t(v) $ ainda são boas aproximações para o centro das fronteiras.
%	
%\begin{figure}[h]
%	\centering
%	\subfigure[Fatia do volume]
...
%\end{figure}
%
%	Por outro lado, vê-se que a segunda derivada média $ h(v) $ de \textit{Kindlmann e Durkin}~\cite{gordon}, identifica uma fronteira muito próxima a $ 170 $, que na verdade é a intensidade da esfera interna. Já a partir de $ t(v) $ obtém-se uma fronteira em $ 195 $, um valor mais próximo do centro exato da fronteira, que é $ 213 $.